...
export async function POST(req: NextRequest) {
  ...
  const sitesToProcess = siteSlug
    ? JEWISH_SITES.filter(s => s.slug === siteSlug)
    : JEWISH_SITES
  ...
  for (const site of sitesToProcess) {
    ...
    const SITE_CONTEXT: Record<string, string> = {
      'jewish-news-now':        'Jewish News Now covers breaking Israel and global Jewish community news. The Facebook group audience: diaspora Jews interested in Israel and Jewish world events.',
      'jewish-property-report': 'Jewish Property Report covers Israeli real estate for diaspora buyers and investors. The Facebook group audience: Jews considering buying property in Israel or making investment decisions.',
      'aliya-today':            'Aliya Today is the definitive guide for Jews making Aliyah (immigrating to Israel). The Facebook group audience: people actively planning, considering, or in the process of making Aliyah.',
    }
...
- Includes 1–3 relevant emojis placed naturally (not at start of every line)
- Does NOT say "Check out this article" or "I found this article"
- Does NOT include the URL in the post body — it will be added below the post
- Ends with a natural call to action like "Full guide in comments 👇" or "Link below 👇" or "Read the full breakdown below"
...
Return ONLY the post text. No preamble, no labels, no quotes around it.`
    ...
    try {
      ...
      posts.push({
        ...
        siteIcon: site.icon,
        article: { ...article, url },
        postBody: '[Generation failed — try again]',
        fullPost: `[Generation failed — try again]\n\n👉 ${url}`,
        tone,
      })
    }
  }
