...
export async function GET(req: NextRequest) {
  ...
  if (!req.headers.get('authorization')?.includes(process.env.CRON_SECRET || '') && req.nextUrl.searchParams.get('secret') !== (process.env.CRON_SECRET || '')) return NextResponse.json({error:'Unauthorized'},{status:401,headers:CORS})
