...
export async function GET() {
  ...
  try {
    ...
    const r = await fetch('https://api.anthropic.com/v1/messages',{
      ...
      body:JSON.stringify({model:'claude-haiku-4-5-20251001',max_tokens:1500,messages:[{role:'user',content:'Write a 3-minute financial podcast script (approx 400 words) between HOST Marcus Webb and GUEST James Richardson CEO of eToro. Format as "HOST: ..." or "GUEST: ..." per line. Topic: regulatory credibility and institutional trust in 2026. Start immediately.'}]})
    })
