...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!site) return {}
  const url = `${BASE}/forum/${slug}`
  return {
    title: site.seo_title || `${site.name} — Trade Community Forum Market Debates Professional Discussion`,
    description: site.tagline || `${site.name} provides professional intelligence on trade community forum market debates professional discussion.`,
    keywords: `${site.name}, trade, community, forum, market, debates, professional, discussion, market analysis, intelligence`,
    robots: 'index, follow',
    ...
    other: { 'application-name': site.name, 'news_keywords': 'Trade Community Forum Market Debates Professional Discussion' },
  }
}
...
export default async function SitePage({
  ...
  const p = site.primary_color || '#ea580c'
  ...
  const schemas = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: site.name, item: `${BASE}/forum/${slug}` },
      ],
    },
  ]
