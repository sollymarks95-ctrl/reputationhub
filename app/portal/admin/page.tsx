...
export default async function AdminPage() {
  const [
    ...
    supabase.from('portal_content').select('*').order('published_at', { ascending: false }).limit(50),
    supabase.from('portal_rankings').select('*').order('current_position'),
    supabase.from('podcast_scripts').select('*').order('created_at', { ascending: false }),
    supabase.from('portal_activity').select('*').order('created_at', { ascending: false }).limit(25),
