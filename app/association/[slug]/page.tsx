...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!site) return {}
  const url = `${BASE}/association/${slug}`
  return {
    title: site.seo_title || `${site.name} — Trade Certification Compliance Industry Standards`,
    description: site.tagline || `${site.name} provides professional intelligence on trade certification compliance industry standards.`,
    keywords: `${site.name}, trade, certification, compliance, industry, standards, market analysis, intelligence`,
    robots: 'index, follow',
    ...
    other: { 'application-name': site.name, 'news_keywords': 'Trade Certification Compliance Industry Standards' },
  }
}
...
export default async function SitePage({
  ...
  const p = site.primary_color || '#1d4ed8'
  ...
  const schemas = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: site.name, item: `${BASE}/association/${slug}` },
      ],
    },
  ]
