...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!site) return {}
  const url = `${BASE}/investdb/${slug}`
  return {
    title: site.seo_title || `${site.name} — Investment Intelligence Private Markets Deal Analysis`,
    description: site.tagline || `${site.name} provides professional intelligence on investment intelligence private markets deal analysis.`,
    keywords: `${site.name}, investment, intelligence, private, markets, deal, analysis, market analysis, intelligence`,
    robots: 'index, follow',
    ...
    other: { 'application-name': site.name, 'news_keywords': 'Investment Intelligence Private Markets Deal Analysis' },
  }
}
...
export default async function SitePage({
  ...
  const p = site.primary_color || '#0f766e'
  ...
  const schemas = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: site.name, item: `${BASE}/investdb/${slug}` },
      ],
    },
  ]
