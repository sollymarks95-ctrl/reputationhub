...
export async function generateMetadata({ params }: { params: Promise<{ slug: string }> }): Promise<Metadata> {
  ...
  if (!site) return {}
  const url = `${BASE}/market-radar/${slug}`
  return {
    title: site.seo_title || `${site.name} — Market Signals Technical Analysis Trading Intelligence`,
    description: site.tagline || `${site.name} provides professional intelligence on market signals technical analysis trading intelligence.`,
    keywords: `${site.name}, market, signals, technical, analysis, trading, intelligence, market analysis, intelligence`,
    robots: 'index, follow',
    ...
    other: { 'application-name': site.name, 'news_keywords': 'Market Signals Technical Analysis Trading Intelligence' },
  }
}
...
export default async function SitePage({
  ...
  const p = site.primary_color || '#b91c1c'
  ...
  const schemas = [
    ...
    },
    {
      ...
      itemListElement: [
        ...
        { '@type': 'ListItem', position: 2, name: site.name, item: `${BASE}/market-radar/${slug}` },
      ],
    },
  ]
